...
-- Test convoy movement system
RegisterCommand('testmovement', function()
    print("^3[Djonluc Evidence Event]^7 ========================================")
    print("^3[Djonluc Evidence Event]^7 🚗 TESTING CONVOY MOVEMENT SYSTEM")
    print("^3[Djonluc Evidence Event]^7 ========================================")
    
    if not eventActive then
        print("^1[Djonluc Evidence Event]^7 ❌ Event not active. Start an event first with /startevidence")
        return
    end
    
    if not eventData or not eventData.route then
        print("^1[Djonluc Evidence Event]^7 ❌ No route set. Set a route first")
        return
    end
    
    -- Check convoy movement configuration
    ...
    -- Check current convoy status
    print("^3[Djonluc Evidence Event]^7 Current Convoy Status:")
    if #convoyVehicles > 0 then
        ...
        for i, vehicle in ipairs(convoyVehicles) do
            if DoesEntityExist(vehicle) then
                ...
                local model = GetEntityModel(vehicle)
                local modelName = Config.Vehicles.evidence_van.model
                
                if model == GetHashKey(modelName) then
                    evidenceVehicle = vehicle
                    print("^2[Djonluc Evidence Event]^7   Evidence Vehicle: Model=" .. modelName .. ", Position=" .. coords.x .. ", " .. coords.y .. ", " .. coords.z)
                else
                    print("^3[Djonluc Evidence Event]^7   Escort Vehicle " .. i .. ": Model=" .. model .. ", Position=" .. coords.x .. ", " .. coords.y .. ", " .. coords.z)
                end
            end
        end
        ...
        if evidenceVehicle then
            ...
            print("^3[Djonluc Evidence Event]^7 Destination Check:")
            print("^3[Djonluc Evidence Event]^7   Current Position: " .. currentPos.x .. ", " .. currentPos.y .. ", " .. currentPos.z)
            print("^3[Djonluc Evidence Event]^7   Destination: " .. destination.x .. ", " .. destination.y .. ", " .. destination.z)
            print("^3[Djonluc Evidence Event]^7   Distance to Destination: " .. string.format("%.1f", distanceToDestination) .. "m")
            
            if distanceToDestination < 15.0 then
                print("^2[Djonluc Evidence Event]^7   ✅ Convoy has reached destination!")
            else
                print("^3[Djonluc Evidence Event]^7   🚗 Convoy is still moving to destination")
            end
        else
            print("^1[Djonluc Evidence Event]^7 ❌ Evidence vehicle not found in convoy")
        end
    else
        print("^1[Djonluc Evidence Event]^7 ❌ No convoy vehicles found")
    end
    
    print("^3[Djonluc Evidence Event]^7 ========================================")
    print("^2[Djonluc Evidence Event]^7 To test movement:")
    print("^3[Djonluc Evidence Event]^7 1. Set a route: /preset police")
    print("^3[Djonluc Evidence Event]^7 2. Start event: /startevidence")
    print("^3[Djonluc Evidence Event]^7 3. Use this command to monitor progress")
...
-- Test ped spawning step by step
RegisterCommand('testspawn', function()
    print("^3[Djonluc Evidence Event]^7 ========================================")
    print("^3[Djonluc Evidence Event]^7 🧪 TESTING PED SPAWNING STEP BY STEP")
    print("^3[Djonluc Evidence Event]^7 ========================================")
    ...
    if #convoyVehicles == 0 then
        print("^1[Djonluc Evidence Event]^7 ❌ No convoy vehicles found. Spawn convoy first")
        return
    end
    
    -- Test ped model loading
    print("^3[Djonluc Evidence Event]^7 Testing ped model loading...")
    local copModel = Config.Peds.escort_cop.model
    local swatModel = Config.Peds.escort_swat.model
    
    local copHash = GetHashKey(copModel)
    local swatHash = GetHashKey(swatModel)
    
    print("^3[Djonluc Evidence Event]^7   Cop model:", copModel, "Hash:", copHash)
    print("^3[Djonluc Evidence Event]^7   SWAT model:", swatModel, "Hash:", swatHash)
    
    -- Test model loading
    RequestModel(copHash)
    RequestModel(swatHash)
    
    local copLoaded = HasModelLoaded(copHash)
    local swatLoaded = HasModelLoaded(swatHash)
    
    print("^3[Djonluc Evidence Event]^7   Cop model loaded:", copLoaded and "✅ YES" or "❌ NO")
    print("^3[Djonluc Evidence Event]^7   SWAT model loaded:", swatLoaded and "✅ YES" or "❌ NO")
    
    -- Test weapon loading
    print("^3[Djonluc Evidence Event]^7 Testing weapon loading...")
    local copWeapon = Config.Peds.escort_cop.weapon
    local swatWeapon = Config.Peds.escort_swat.weapon
    
    local copWeaponHash = GetHashKey(copWeapon)
    local swatWeaponHash = GetHashKey(swatWeapon)
    
    print("^3[Djonluc Evidence Event]^7   Cop weapon:", copWeapon, "Hash:", copWeaponHash)
    print("^3[Djonluc Evidence Event]^7   SWAT weapon:", swatWeapon, "Hash:", swatWeaponHash)
    
    -- Test spawning a single ped
    ...
    if testVehicle and DoesEntityExist(testVehicle) then
        print("^3[Djonluc Evidence Event]^7   Using test vehicle:", testVehicle)
        
        -- Try to spawn a test ped
        local testPed = SpawnEscortPed(Config.Peds.escort_cop, testVehicle, true)
        if testPed then
            print("^2[Djonluc Evidence Event]^7   ✅ Test ped spawned successfully:", testPed)
            -- Clean up test ped
            DeleteEntity(testPed)
            print("^3[Djonluc Evidence Event]^7   Test ped cleaned up")
        else
            print("^1[Djonluc Evidence Event]^7   ❌ Test ped spawn failed")
        end
    else
        print("^1[Djonluc Evidence Event]^7   ❌ No valid test vehicle found")
    end
    
    print("^3[Djonluc Evidence Event]^7 ========================================")
end, false)

-- Test spawn coordinates and vehicle positioning
RegisterCommand('testcoords', function()
    print("^3[Djonluc Evidence Event]^7 ========================================")
    print("^3[Djonluc Evidence Event]^7 📍 TESTING SPAWN COORDINATES")
    print("^3[Djonluc Evidence Event]^7 ========================================")
    ...
    print("^3[Djonluc Evidence Event]^7 Vehicle Positions and Spawn Points:")
    for i, vehicle in ipairs(convoyVehicles) do
        if DoesEntityExist(vehicle) then
            local coords = GetEntityCoords(vehicle)
            ...
            print("^3[Djonluc Evidence Event]^7 Vehicle " .. i .. ":")
            print("^3[Djonluc Evidence Event]^7   Model:", model)
            print("^3[Djonluc Evidence Event]^7   Position:", coords.x, coords.y, coords.z)
            print("^3[Djonluc Evidence Event]^7   Heading:", heading)
            
            -- Calculate spawn position for ped
            local offsetX = math.sin(math.rad(heading + 90)) * 2.0
            local offsetY = math.cos(math.rad(heading + 90)) * 2.0
            ...
            print("^3[Djonluc Evidence Event]^7   Calculated Ped Spawn Position:", spawnPos.x, spawnPos.y, spawnPos.z)
            print("^3[Djonluc Evidence Event]^7   Offset from vehicle: X=" .. offsetX .. ", Y=" .. offsetY .. ", Z=1.5")
            
            -- Check if spawn position is valid (not underground, not in water, etc.)
            local groundZ = 0.0
            local foundGround, groundHeight = GetGroundZFor_3dCoord(spawnPos.x, spawnPos.y, spawnPos.z, true)
            if foundGround then
                groundZ = groundHeight
            end
            
            print("^3[Djonluc Evidence Event]^7   Ground level at spawn:", groundZ)
            print("^3[Djonluc Evidence Event]^7   Spawn height above ground:", spawnPos.z - groundZ)
            
            if spawnPos.z - groundZ < 0.5 then
                print("^1[Djonluc Evidence Event]^7   ⚠️ WARNING: Spawn position too close to ground!")
            end
...
-- Simple ped spawn test (basic)
RegisterCommand('testsimpleped', function()
    print("^3[Djonluc Evidence Event]^7 ========================================")
    print("^3[Djonluc Evidence Event]^7 🧪 TESTING SIMPLE PED SPAWN")
    print("^3[Djonluc Evidence Event]^7 ========================================")
    ...
    print("^3[Djonluc Evidence Event]^7 Player position:", playerCoords.x, playerCoords.y, playerCoords.z)
    
    -- Test basic ped spawn at player location
    local pedModel = "s_m_y_cop_01"
    local pedHash = GetHashKey(pedModel)
    
    print("^3[Djonluc Evidence Event]^7 Testing ped model:", pedModel, "Hash:", pedHash)
    ...
    while not HasModelLoaded(pedHash) and (GetGameTimer() - startTime) < 5000 do
        Citizen.Wait(0)
    end
    
    modelLoaded = HasModelLoaded(pedHash)
    print("^3[Djonluc Evidence Event]^7 Model loaded:", modelLoaded and "✅ YES" or "❌ NO")
    
    if modelLoaded then
        -- Spawn ped slightly in front of player
        local spawnPos = vector3(playerCoords.x + 3.0, playerCoords.y, playerCoords.z)
        print("^3[Djonluc Evidence Event]^7 Spawning ped at:", spawnPos.x, spawnPos.y, spawnPos.z)
        
        local testPed = CreatePed(4, pedHash, spawnPos.x, spawnPos.y, spawnPos.z, 0.0, true, true)
        
        if DoesEntityExist(testPed) then
            print("^2[Djonluc Evidence Event]^7 ✅ Basic ped spawned successfully:", testPed)
            
            -- Set basic properties
            SetEntityAsMissionEntity(testPed, true, true)
            SetPedMaxHealth(testPed, 200)
            SetEntityHealth(testPed, 200)
            ...
            print("^1[Djonluc Evidence Event]^7 ❌ Basic ped spawn failed")
        end
        
        -- Clean up model
        SetModelAsNoLongerNeeded(pedHash)
    else
        print("^1[Djonluc Evidence Event]^7 ❌ Failed to load ped model")
    end
    
    print("^3[Djonluc Evidence Event]^7 ========================================")
end, false)

-- Test vehicle spawning specifically
RegisterCommand('testvehicles', function()
    print("^3[Djonluc Evidence Event]^7 ========================================")
    print("^3[Djonluc Evidence Event]^7 🚗 TESTING VEHICLE SPAWNING")
    print("^3[Djonluc Evidence Event]^7 ========================================")
    ...
    print("^3[Djonluc Evidence Event]^7 Player position:", playerCoords.x, playerCoords.y, playerCoords.z, "Heading:", playerHeading)
    
    -- Test spawning each vehicle type
    local testVehicles = {
        {name = "Police Car", model = "police", offset = 5.0},
        {name = "FBI SUV", model = "fbi2", offset = 10.0},
        {name = "Evidence Van", model = "stockade", offset = 15.0}
    }
    
    local spawnedVehicles = {}
    
    for i, testVehicle in ipairs(testVehicles) do
        print("^3[Djonluc Evidence Event]^7 Testing:", testVehicle.name)
        
        -- Calculate spawn position
        local spawnPos = vector3(
            playerCoords.x + testVehicle.offset,
            playerCoords.y,
            playerCoords.z
        )
        
        print("^3[Djonluc Evidence Event]^7 Spawning at:", spawnPos.x, spawnPos.y, spawnPos.z)
        
        -- Spawn vehicle
        local vehicle = SpawnVehicle(testVehicle.model, spawnPos, playerHeading)
        
        if vehicle and DoesEntityExist(vehicle) then
            print("^2[Djonluc Evidence Event]^7 ✅", testVehicle.name, "spawned successfully:", vehicle)
            ...
            local model = GetEntityModel(vehicle)
            print("^3[Djonluc Evidence Event]^7   Position:", coords.x, coords.y, coords.z)
            print("^3[Djonluc Evidence Event]^7   Model Hash:", model)
            
        else
            print("^1[Djonluc Evidence Event]^7 ❌", testVehicle.name, "spawn failed")
        end
    ...
    -- Clean up test vehicles after 10 seconds
    if #spawnedVehicles > 0 then
        print("^3[Djonluc Evidence Event]^7 Test vehicles will be cleaned up in 10 seconds...")
        ...
        Citizen.CreateThread(function()
            ...
            for _, vehicle in ipairs(spawnedVehicles) do
                if DoesEntityExist(vehicle) then
                    ...
                    print("^3[Djonluc Evidence Event]^7 Test vehicle cleaned up")
                end
            end
            spawnedVehicles = {}
        end)
